<template>
	...
	<view class="recommendations-container">
		<view class="recommendations-content-container" v-for="(item, index) in recommends" v-if="recommends.length >= 1" :key="index" @click="targetDetails(item.id)">
			<image class="" :src="item.poster" />
			<view class="recommendations-content">
				<text class="recommendations-content-desc" selectable="false" space="false" decode="false">【{{ item.city_name }}站】{{ item.name }}</text>
				...
				<text class="recommendations-content-name" selectable="false" space="false" decode="false">{{ item.venue.name }}</text>
				<view class="price" selectable="false" space="false" decode="false">
					{{ item.lower_price / 100 }}
					<text class="min-text" selectable="false" space="false" decode="false">元起</text>
				</view>
				<button class="goupiao" v-if="goupiaoOnoff" @click.stop="gouPiao">购票</button>
			</view>
		</view>
		...
		<view class="no-content" v-if="recommends.length < 1">
			<image class="" src="https://novelsys.oss-cn-shenzhen.aliyuncs.com/ticket/static/image/componentImg/no-content.png" />
			<view class="text" selectable="false" space="false" decode="false">暂无相关内容</view>
		</view>
